...
int getTotalSize(std::map<std::string, int> folderSizes){
  ...
  int requiredDiskSize{30000000};
  int usedDiskSize{folderSizes[""]};
  int unusedDiskSize{totalDiskSize - usedDiskSize};
  int minRequired{requiredDiskSize - unusedDiskSize};
  int min{usedDiskSize};
  for (auto el : folderSizes)
  {
    if (el.second > minRequired && el.second < min)
      min = el.second;
  }
  return min;
}
