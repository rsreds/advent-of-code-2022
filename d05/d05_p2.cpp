...
std::vector<char> getTopCrates(const char *filename)
{
  ...
  while (std::getline(file, line))
  {
    ...
    stacks[toStack -1].insert(stacks[toStack-1].end(),
                              stacks[fromStack-1].end()-howMany,
                              stacks[fromStack-1].end());
    stacks[fromStack - 1].erase(stacks[fromStack - 1].end() - howMany, stacks[fromStack - 1].end());
